 ...
 * ⭐ 那条 400 已经没了，particle 换成了一个**更难发现的坏法**：图散在多轮历史里时它会跑偏
 * （08-30 消融各 10 发：答非所问 5/10、其中 3/10 把 `<tool_call>bash ...` 当正文吐；zai 0/10）。
 * 行也随之从点死一家改成偏好序 `vendors:['zai','particle']`。
 *
 * 所以这个探针的形状是**故意挑的**：三张图分三轮发 + 简短的 assistant 回合 —— 这正是 agent
 * 循环的样子，也正是唯一能把两家分开的形状。⛔ 别改成"三张图塞同一条消息"：那样 particle
 * 也 0/10 不出事，探针会变成恒绿的摆设（我第一趟 30/30 全绿就是这么被骗过去的）。
 *
 * 两条判据：
