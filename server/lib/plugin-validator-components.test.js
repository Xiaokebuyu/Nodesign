...
describe('解压后大小（09-08 zip 炸弹）', () => {
  it('压缩后 8MB 以内、解压后超单文件上限的 entry 拒收，而且不解压它', async () => {
    const { validateSkillUpload, LIMITS } = await import('./plugin-validator.js');
    const JSZip = (await import('jszip')).default;
    const zip = new JSZip();
    zip.file('.claude-plugin/plugin.json', JSON.stringify({ name: 'bomb', version: '1.0.0', description: 'x' }));
    zip.file('skills/bomb/SKILL.md', '---\nname: bomb\ndescription: x\n---\n# bomb\n');
    zip.file('skills/bomb/data.json', Buffer.alloc(LIMITS.ENTRY_MAX_BYTES + 1024, 0x20));   // 全是空格，压得极小
    const buf = await zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE', compressionOptions: { level: 9 } });
    expect(buf.length).toBeLessThan(LIMITS.ZIP_MAX_BYTES);
    const r = await validateSkillUpload(buf);
    expect(r.ok).toBe(false);
    expect(r.errors.join(' ')).toMatch(/解压后/);
  });
});
