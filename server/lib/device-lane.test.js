...
describe('fitFor 出的版式', () => {
  /**
   * ⭐ 2026-09-01 叠纸刀 7（站主拍板「手机端的默认缩放可以设置为 50%」）。
   *
   * 08-28 那一档是「一件 = 一屏」：宽 = 屏宽 − 48、高 1.6 屏，隐含 100% 缩放。
   * 前提是**手机上唯一可用的姿势是一件占满一屏、竖着翻** —— 叠纸把那个前提换掉了
   * （一摞纸原地翻页，屏幕不用再当纸筒使），所以纸放大到两个屏幕，50% 下整页入镜。
   *
   * ⚠️ **只有纸变大，别的一概不变**：正文栏宽 `colW` 仍是一屏。两个数分开钉，
   * 因为把它们合成一个正是这一刀最容易做错的地方 —— 让 agent 按纸宽写，用户放大
   * 到 100% 读一行就超出屏幕，横向滑动又回来了。
   */
  it('手机：纸 = 两个屏幕（50% 基准），正文栏宽仍是一屏', () => {
    const f = fitFor(vpOf(PHONE));
    expect(f.lane).toBe('phone');
    expect(f.column).toBe(true);
    expect(f.screen).toEqual({ w: 390, h: 664 });
    expect([f.w, f.h], '纸 = 屏幕 / 0.5').toEqual([780, 1328]);
    expect(f.colW, '正文栏宽仍锁死一屏（屏宽 − 48）').toBe(342);
    // 一张 780 的纸并排放得下两栏 342（342 * 2 + 24 = 708），每栏放大到 100%
    // 都正好一屏宽 —— 这就是「纸变大而字不变」换来的东西
    expect(f.colW * 2 + 24).toBeLessThanOrEqual(f.w);
  });
  ...
  it('⭐ 屏幕像素跟缩放无关 —— 用户缩到 0.3 倍，版式建议不该跟着变', () => {
    ...
    expect(fitFor(zoomed).w).toBe(780);
    expect(fitFor(zoomed).colW).toBe(342);
  });
