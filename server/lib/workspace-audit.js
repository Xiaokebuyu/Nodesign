...
export async function auditWorkspace(projectId, { sharedRoot = getSharedDir(projectId), board = null } = {}) {
  ...
  return { projectId, sharedRoot, objects: ids.length, zones: zones.length, checked: checked.length, dangling, unseated: unseated.slice(0, 50), unseatedCount: unseated.length, files: files.length };
}
