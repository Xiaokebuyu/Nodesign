...
// rp（演出）项目的档案模板（2026-08-28；08-30 随角色子代理停用改口径）。
// ⚠️ 原注：这份文件**每个角色子代理也强制吃**（项目 CLAUDE.md 进所有子代理上下文，
// omitClaudeMd 透不过去 —— 08-26 实测）。08-30 起主 agent 代演全场，暂时没有子代理
// 在读它，但这条约束等角色回来还成立，别按"只有一个读者"重排栏目。
// 详细设定别塞 —— 放世界书文件按需 grep，这份是每轮都重发的常驻开销。
export const DEFAULT_CLAUDE_MD_RP = `# 演出档案

这份文件每次会话都完整进入 agent 的上下文，但**演出进程不读它**。正式的故事在演出显示器上演：
世界、规矩、人物、规则都在那个故事自己的文件夹里（open_stage 交的设定、cast_role 写的卡），
⛔ 别把它们抄一份到这里。这里只放跨故事的东西。改硬约束要经用户点头。

## 玩家
（怎么称呼他、他扮演过谁、偏好：慢一点还是快一点 / 多对白 / 第一人称 / 不喜欢什么。开场问答的答案落在故事的设定里，这里只记跨场的习惯。）

## 在演的故事
（一行一场：文件夹名 · 一句话 · 状态。演到哪了别记这里，用 stage_status 看。）
`;
// 09-06 起 RP 项目的 CLAUDE.md 只是 agent 的跨故事档案：故事本体（设定 / 卡 / 规则 / 记忆）在故事文件夹里，
// 演出进程不读这份文件（stage/session.js 不加载 CLAUDE.md）。旧的四节骨架（世界观锚 / 台面规矩 / 人物 / 称呼）已退役。
