...
这份文件每次会话都完整进入 agent 的上下文，但**演出进程不读它**。正式的戏在演出显示器上演：
世界、规矩、人物、规则都在那场戏自己的文件夹里（open_stage 交的台面、cast_role 写的卡），
⛔ 别把它们抄一份到这里。这里只放跨故事的东西：玩家是谁、他怎么喜欢玩、这个项目里有哪几场戏。
下面三节是**兜底代演**（你自己在对话里演一两拍）时才用的骨架，正式的戏留空就行。改硬约束要经用户点头。
