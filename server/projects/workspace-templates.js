...
⛔ 别把它们抄一份到这里。这里只放跨故事的东西：玩家是谁、他怎么喜欢玩、这个项目里有哪几个故事。
下面三节是**兜底代演**（你自己在对话里演一两段）时才用的骨架，正式的故事留空就行。改硬约束要经用户点头。

## 玩家
（怎么称呼他、他扮演过谁、偏好：慢一点还是快一点 / 多对白 / 第一人称 / 不喜欢什么。开场三问的答案落在戏的台面里，这里只记跨场的习惯。）
...
从酒馆世界书搬来的常驻条目也放这，十条以内 —— 其余降级成触发文件。）

## 规矩
（这一场怎么玩：想跟谁演（一对一 / 一群人）/ 难度档（爽·写实·严酷）/
代笔档（全代入·大纲）/ 骰子规则 / 文风与叙事禁令 / 禁区题材。）

## 人物
（在场人物一览：每人三行核心 —— 是谁、什么关系、绝不做什么。这里是名录；
每个人完整的人设在 角色/<名>/角色卡.md（cast_role 写的那张），写他的话之前先读。）
`;
// ⚠️ 上面四节的节名是「演出档案骨架」的唯一真相 —— blackboard-rp SKILL 的酒馆导入
// 落点表按同一套节名指路（08-28 对账：此前两处各写一套骨架，字段对不上）。改节名两处同改。
