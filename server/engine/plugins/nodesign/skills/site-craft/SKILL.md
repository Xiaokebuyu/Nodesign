...
name: site-craft
version: 0.5.0
description: NoDesign 站点设计方法论 —— 静态站点（手写起步，CDN 库与构建按需），先把用户脑子里的感觉翻译成一个真实存在的风格名给他挑，再对齐一页的气质，然后铺开全站。含骨架推导、风格命名与翻译词典、排版与色彩系统、图像手法（抠图叠层）、中文排版基线、固定画幅页面（海报 / 笔记图 / 歌词页 / 演示，画幅按题材默认不先问）、反默认清单、构建道配方（React / vite / Tailwind / 组件库站点）。当用户要做网站 / 官网 / 个人站 / 落地页 / 作品集 / 博客 / landing page / 多页站点、海报 / 长图 / 演示 / 单页报告，或要做带功能的站（登录 / 列表 / 表单 / 后台 / 管理界面 / 可运行原型，数据用模拟的），或要"能发出去的一个地址"时使用。不用于 Word 文档（那是 docx-craft）。
---
...
跟我想的不一样"。试作放**工作区根**的 `_drafts/<名字>.html`（不是站点文件夹里的），各自渲成卡并排给用户挑。
...
- 「弄清一件事」→ 概览 + 分主题子页，导航要清晰，不做无谓的技术展示
...
只会把你推向模型均值 —— "温暖克制的现代感"做出来就是那张 AI 默认落地页；"粗野主义"
做出来立刻就有样子，因为它在现实里有实物、有做法、有可仿照的对象。反默认清单
说的是**别去哪**，风格名说的是**去哪**，这两件是一对。

这一步的产出：**一个用户点过头的名字**。

1. 从他的话里提取特征：他提到的东西、举的例子、否掉的东西，以及这个站点的用途
   （个人站 / 作品集 / 同人本宣传 / 长文阅读页各有自己的风格谱系）。
   ⭐ **题材自带视觉传统的，候选从那个传统里来**：站的主题是一首歌、一个 IP、
   一个品牌、一个年代或地域时，它多半有官方视觉 / 既成的粉丝审美 —— 先搜一眼
   它的官方视觉再命名（风格词条在你脑子里，**题材的视觉事实不在**）。
   此前有一次：仅凭词条印象给出的三个候选全部偏离，用户提出
   "要不要搜搜它的设计风格"，搜索后才发现主色是另一组 —— 这一步应由你主动提出
...
⭐ **站点还要看参考站**：风格搜完知道它长什么样，**版面结构**（开场是刊物封面
还是 banner、正文页两侧的空白怎么用、滚动有没有惯性）只有进真站才看得到。
实际对比表明：仅读搜索结果的文字描述收效有限，直接看三个参考站十分钟即可定位到具体差距。

⭐ **想到活例子就提出来，深逛之前先征询一句**。命名或试作时想到某个真站 / 某类站
正是这个风格的活例子，应主动提出 —— 例如："需要我参考一下 XX 这类站点，把它的
配色、字体和动效整理出来作为依据吗？"（聊天里一句话就行，不用开卡片）。深逛要花几分钟，
先问再展开；他说不用就直接做，他丢了链接或参考图就直接用不再问。**提议是主动的，
展开是经他点头的** —— 完全不提，等于让用户自己去想到这条路径。

**怎么看**：搜索→打开→翻内页→一趟 `browser_batch` 采回来的流程在平台协议
「上网看东西」一节，这里只补站点特有的三条：

- ⭐ `browser_capture` 不只截图：**调色板**（带用途）、**字体**（字族 + 实际字号字重行高）、
  某一块的 **CSS 片段**、**结构三数**，以及 **动效清单**（`kinds:['motion']`：这站靠什么在动 ——
  @keyframes / transition / 滚动驱动动画 / scroll-snap / sticky / GSAP+ScrollTrigger 每个
  trigger 的 start·end·scrub / Lenis 劫持 / 哪些元素滚动入场）。想看它**动起来的样子**用
  `browser_screenshot { frames:[…], scrollBy: … }`（格数 2~30：6~10 看细节、12~16 看整段、
  20~30 看长滚动的节奏）出一张胶片条 + 元素探针。"看全"一个参考站 = 版面（截图）+
  用料（palette/fonts/css）+ 骨架（skeleton）+ 动效（motion + 胶片条），四样齐了再下手。
- ⭐ 结构三数正好是下面「用户说无聊的时候」那节要数的三样。**拿参照站的数字跟你
  正在做的那个站比**：「这个站 6 种节的形状，你现在的站 1 种」比「我觉得有点单调」
  能落地得多。（那三个数是启发式，当对照不当真相。）
- 遇到人机验证无法通过时用 `browser_request_help` 请用户协助确认 —— 但先想清楚是不是
  真需要：部分校验依据的是服务器 IP，用户重复操作同样无法通过，那就换一个参考站并跟用户说明。
...
他已经点了名字（"我要日式极简"）的，跳过前两步直接进试作；直接提供了参考图的，
读图读出那是什么风格、跟他确认一下名字，再进试作。名字定下来之后落锚进长期
记忆（一条 `type: project` 记忆，名字打头如「风格锚 = 杂志编辑风」，取值写进正文）；
...
**这三个数决定成败**（写进 `:root`，锚定时固化；**先定这三个数再配色**，反过来做
配色反复调整也难以对味）：
...
生成图落在**工作区根**的 `assets/generated/`，而站点住自己的文件夹 —— 要么把 webp
拷进 `<站名>/assets/` 再写 `assets/x.webp`，要么写 `../assets/generated/x.webp`
（导出时系统会归一）。没有合适的图时，大字排版本身就是视觉主角，
不要用 emoji 或通用免费图充数。

## 动效预算

CDN 库可按需引入（gsap / lenis / three / alpine / htmx），但动效是调味不是主菜：

- **一页最多两种动效语言**：比如"滚动淡入 + hover 微移"就是全部。首屏可以有
  一处大的（打字机 / 视差 / 3D 背景），后面全篇克制
- 滚动揭示别每个元素都做，按**节**做；时长 0.4~0.7s，缓动统一一个函数
- 需支持 `prefers-reduced-motion`，一条 media query 即可
- 动效永远不该让人等：进场动画挡住内容超过 1s 就是负资产

## 中文排版基线

- 标点跟**正文主体语言**走：中文正文用全角（，。：；？！……），半角逗号冒号在中文里会破坏版面一致性；
  英文正文用半角加一个空格，全角标点混进英文句子里同样破坏一致性。混排按主体那一种算
...
里的分片）。简单站点到此即可，不必为其引入工程化。构建道（React /
打包器 / 生成脚本）什么时候值得上、如何配置才能与平台约束兼容（`base: './'`、
...
需要与用户对齐的口径：**本产品提供设计与交付，不提供托管运行** —— 交付物是完整前端，附一份
「需由您的后端实现的接口」清单。

样式建议集中在一份 CSS 里 —— 用户自己才改得动；utility class（Tailwind 等）
散在几百个标签上，改行高要改一百处。用户点名要就用。（构建道上的取舍不同，
...
## 固定尺寸的页面（海报 / 笔记图 / 歌词页 / 演示）

站点的页面默认是响应式的、可滚动的。有一类需求要的是**固定尺寸的一屏**：海报、社交平台的笔记图、歌词视觉页、单页报告、演示。它们仍然是站点里的页面，只是加了两个属性：固定的画幅，以及页与页之间按屏切换。不需要另一种产物形态。

**画幅按题材默认，不要先问用户。** 用户说"海报"就直接开工，第一版出来之后再问要不要换比例，这比开工前问卷快得多，也让用户先看到东西。默认值：

| 题材 | 默认画幅 |
|---|---|
| 海报、笔记图（小红书类）、歌词页 | 1080×1440（3:4 竖版） |
| 手机端宣发、竖版长图 | 1080×1920（9:16） |
...
**固定画幅的字号跨度比网页大**。一屏是隔着距离看的：核心句 72 到 160px，正文 20 到 28px，注脚 14 到 16px。层级用字号、字重、颜色深浅三个维度一起做。块间距明显大于行间距，一屏的呼吸感靠留白，不靠缩小字号。

**图像**：全幅大图只压文字落脚的局部（`patterns/image-led-cover.md`）；`generate_image` 出的主体可以 `remove_background` 抠成透明叠层，人物半身像出血压边（`patterns/portrait.md`），物件散落在大字周围。抠图用 `filter: drop-shadow(...)` 贴地，不用 box-shadow。

**先做一屏给用户看，再铺其余**。海报只有一屏，做完直接给看；演示先做封面和一页典型内页，用户认可气质之后再铺开。铺完再改等于重做。
...
本节来自一次实际案例：用户三次表达不满，前两次的处理方向都不完整。

1. "为什么不多加些超棒的特效" → 增加了滚动揭幕、hover 微动、胶片颗粒。**无效。**
2. "为什么不生成专用的图" → 补了角色一致的配图。有用，但不解决根本。
3. "太简单太无趣了" → 这次才诊断对：站是 3 页、1 篇文章、每一节都是"左图右字"
   的同一种节奏，读者三屏到底。

**"无聊"八成是骨架太薄，不是皮肤不对。** 而增加动效只是给薄骨架加装饰 —— 骨架的厚度
并没有变化。

诊断次序，先数三样可数的东西：

- **页数**：3 页和 8 页是两种体验。一页承载不了两个重心，强行合并只会失焦
...
**动效是最后一步，不是第一步。** 骨架立住之后，动效才能让它更顺；骨架单薄时，
动效只是让单薄动起来。
...
> 起手：没有模板，从骨架和风格名自己写起（模板会让所有站点趋同，
> 已停用）。硬要求只有产物根上的 `index.html`；样式文件的拆法和名字按站定。两条让预览出错的硬规则 —— `<meta name="viewport">`
