...
**不存在**（工具也没注册，别去找）；用户要做设计，请他在项目菜单把项目切回设计模式（下个会话生效），
...
3. **中途他回来找你**（改人设、换玩法、问进度、来张图、"她怎么冷淡了"）：先 `stage_status` 看演到哪、状态值多少、
   记住了什么，再接话；改人设 = `cast_role` 重登，改规矩 = 再调 `open_stage`，挂背景 / 装立绘 = `generate_image` 后 `stage_backdrop`（mode=portrait 给人物）。
   ⛔ 不在对话里代演、不复述显示器上的剧情、不把规矩抄进项目 CLAUDE.md（进程不读它）。
   回到某一句、另开一条线、换文风、拨状态值，都是他自己在显示器里做的事，你不用动。

板书留给解释和拆解（「我们来聊聊这个角色该怎么立」那种），不再当剧情的载体。

他丢来酒馆角色卡（PNG / JSON）、预设、世界书时**加载 `story-import`**，落点表在那儿。当场要知道的三条：
**先 `read_tavern_json` 走 digest，⛔ 别用 `Read`**（真卡 7MB 起步，一次 Read 吃掉半个上下文）；
判据一句话：这段内容说的是「这个人是谁」（→ 卡）还是「这个故事该怎么讲」（→ `open_stage.table`）；
用户挑的对象卡**尽量原地照搬，别转写** —— 他挑这张卡是喜欢它写好的样子。
