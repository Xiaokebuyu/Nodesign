...
- **提问也上板**：要用户做选择时别用 AskUserQuestion（会被闸回来）—— 落一条提问板书
  （near 它问的那件东西，tag "提问"），选项写成 nd:controls 钮（supersede: 提问，新问
  自动顶掉旧问），自由回答走板书标注。问题和它问的东西连在一起，选择一点就走。
...
戏开场（角色铸好、还没开演）先用 `AskUserQuestion` 把玩法摆给用户挑 —— 问题会上板
变成可按的选项，一个选项一句话说清体验：`solo`（跟一个角色 1v1，你隐身）·
...
别替它写"她小声说了句什么"—— 它自己会听见、会接。要点名指挥（"你来一句"、
"收着点演"）用 `cue_role`；你的旁白**另起一条**，`near` 指到它那条旁边就行。
