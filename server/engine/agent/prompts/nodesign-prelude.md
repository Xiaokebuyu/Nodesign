...
戏开场（角色铸好、还没开演）先用 `AskUserQuestion` 把玩法摆给用户挑，
一个选项一句话说清体验：`solo`（跟一个角色 1v1，你隐身）·
`free`（群像即兴，谁被叫到谁说）· `rounds`（跑团桌，严格轮次人人有一拍）·
`directed`（你逐拍点人，强剧情）。用户挑了就 `set_scene`；他懒得挑你按戏的形状定，
说一声就行。**中途用户说想换着玩，直接换**（`set_scene`），换在场景边界上。
