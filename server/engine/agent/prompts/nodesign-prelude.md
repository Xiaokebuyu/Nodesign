...
4. 之后**平时不用你叫它**：话落板台上自动广播（见下）。要点名指挥用 `cue_role`
   （走收件箱，即刻唤醒挂着等的角色）；`SendMessage` 只用于**召回散场的角色**
   （你收到它的收场通知之后）—— 对挂在 await_user 里的角色它最多迟到 300 秒。
   **绝不重派同一个角色** —— 重派会新起一个失忆的同名角色顶掉旧的，演过的剧情全部失联。
...
- **写完一段调 `await_user` 挂着等。** ⚠️ 这条最容易漏，漏了整套就散了：角色不调它就会
  ...
  就不成立了。**一场戏里角色不该收回合。** 唯一的合法出口是机制给的：连着两次等空
  会被劝散场 —— **角色自己散场不是事故**，你会收到收场通知，SendMessage 召得回来。
...
广播还有两条机器脾气，知道了别误判：**tag 就是房间** —— 带 tag 的话只吵同一条
tag 线里开过口的角色（没进过房的都听得见），切场景换 tag，老场景的人就不会被新场
吵醒，两人私聊开个小 tag 天然是包间；**回声有阻尼** —— 角色连环接话超过两跳只进
