...
> （那份存档 08-24 已删：全仓无引用的孤儿文件；如需恢复 gateway，请从 git 历史中取回）。
...
| 事实 | 对你的后果 |
|---|---|
| **prompt 逐字下传，零改写** | 桥接层写死了禁止改写/增删/翻译/润色。你写什么模型收到什么。该结论已由日志逐字节核验两次，**不用再自行验证** |
...
裁到需要的形状；或者干脆用 CSS 画，不必为一条色带花费 60 秒生成。
...
信息图）来定完成度；复杂请求用 § B3 的短标签行，不要堆成一整段。
...
| logo-brand | 简单可缩放；强剪影、留白平衡；不做装饰性花哨处理 |
...
⚠️ **该调用链路是无状态的**，所以"接着上一张改"必须显式做两件事：把上一张图放进
...
- `variationOf` 就是编辑基底（自动成为第 1 张 reference），`change` 是唯一要动的那处，
  ...
  自动追加通用禁止项（不凭空加背带/发饰、缺席也要保持 —— 均为实际出现过的失败情形）。
...
| 整页 cover / hero | ❌ 背景本身是设计的一部分，去除后会损失设计意图 |
