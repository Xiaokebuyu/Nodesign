...
export const MODELS_BUILTIN = Object.freeze([
  ...
  // （有缓存、并发不紧），只是接默认之前得先解决瞎图那 7~10%（或者接受它）。
  {
    ...
    // ③ 单点：这行 `bodyExtra.vendor: 'zai'` 点死了厂商，**没有故障转移**。它当默认之后，
    //    那一家挂 = 全站默认路径挂。要转移是 `vendors: ['zai','baseten']`，但 baseten 同模型贵 10 倍。
    ...
    api: {
      ...
      // 08-28 点死 zai 的理由（particle 对多图回 400）已作废，复测 36/36 全通。zai 仍排第一是因为
      // 这行是默认行、稳字优先（particle 的实测全在下面「快线」那行）。
      // ⛔ **baseten 不许进列表**：同一发请求 usage.cost $0.000626，是 particle 的 48 倍、zai 的 11 倍。
      // ⚠️ `vendors` 是**偏好序不是故障转移链**：列谁在前就一直落谁（30/30、8/8 两向都验过），且拿一个
      //    zai 必拒的请求试 `['zai','baseten']` 回的是 400 而不是转到 baseten；5xx 会不会转移没验到。
      //    ⭐ 不存在的家名会被静默过滤（`['nope','zai']` → zai 服务），不报错。
      // ⛔ 「不指定就能不排队」是假的：不点名实测 20/20 全落 zai，而慢的正是 zai。⚠️ 网关的默认选择
      //    会自己变：08-28 裸请求 8/8 落 particle，08-30 是 20/20 落 zai。
  ...
  },
  {
    ...
    // ⭐⭐ 凭什么单开一行（08-30 晚**改口**，原来这里写的是"快 5 倍"，那是小提示上的假象）：
    //   真实体量实测（6.5 万 token 前缀、缓存已热、4 条会话同时发一轮 = 08-26 量到的站点峰值）：
    //   **墙钟 particle 3111ms / zai 3874ms，每轮成本 $0.000816 / $0.000836** —— 快约 20%、价钱打平。
    //   ⛔ 之前记的"8 并发快 5 倍、便宜 4 倍"是在几百 token 的提示上量的，那测的是排队调度不是
    //   真会话（真会话每轮几万 token 且缓存热着）。**小提示上的跑分不能拿来讲真会话的故事。**
    // ⭐⭐ **换厂商 = 缓存全丢**（08-30 实测）：6.5 万前缀在 zai 上焐热（cached 63232、$0.000192/轮），
    //   同一前缀换到 particle → cached **0**、$0.000951 **贵 5 倍**；反向同理；换回来 zai 仍命中 ——
    //   缓存没被毁，只是**各家一份、跨不过去**。这就是 08-27「不做请求级动态路由」那条规矩的地基。
    // ⚠️ particle 的缓存**只认"长大"不认"换尾巴"**：消息列表逐轮追加（真会话的样子）→ 7/7 命中；
    //   列表长度不变只换最后那句 → 1/7（zai 两种都 7/7，且 65k 以下 particle 干脆不进缓存）。
    //   本站的会话是逐轮追加的，所以撞不到；但**任何重写历史的动作（compact 除外，那两家都冷）
    //   在这家的代价比在 zai 大**。
    // ⚠️ **唯一的弱项，以及为什么对本站不成立**（08-30 分四轮量，中间翻过两次车）：particle 在
    //   「图散在多轮历史里 + **请求没声明 tools**」时只看得见最后一张图（20 发挂 8 发，zai 20/20）。
    //   ⭐ 声明了 tools 就没这回事（同消息 + tools 数组 20/20；真会话形状连测三轮 46/46）——
    //   本站的请求永远带 tools（agent SDK 必然声明 MCP 工具），撞不到。
    //   ⛔ 别拿"单条塞多图"复验：那形状两家都零失败，是恒绿的摆设。也别信目录自报的能力位和
    //   上限（它说 particle max_output 131000，拿 131050 试它照收）。
    // ⛔ 08-28 起写在上一行的「particle 一次只收一张图、会回 400」已作废（上游修掉了，36/36 全通）。
    id: 'glm-5.3-flash-fast', window: 1_000_000, brand: 'glm',
    // label 第一段跟上面那行同名是**故意**的：compactLabel 会自己发现撞名、给两行都留两段。
    select: { label: 'GLM-5.3-Flash · 快线', desc: '有视觉 · 1M 上下文 · 极便宜 · 另一家部署，高峰期略快' },
    api: {
      upstream: 'merge', wireModel: 'zai/glm-5.3-flash',
