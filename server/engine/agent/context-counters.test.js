...
describe('takeCarry / addCarry —— 半截续接的账结转', () => {
  const fill = (ctx, n) => {
    ...
    ctx.counters.modelUsage = { 'glm-5.3-flash': { inputTokens: n, outputTokens: n * 2, cacheReadTokens: n * 3, cacheCreateTokens: n * 4, costUsd: n / 100 } };
  };
  ...
  it('modelUsage 按模型逐项相加（run_model_usage 落库口径，日限闸门读它）', () => {
    ...
    fill(ctx, 50);
    ctx.addCarry(carry);
    expect(ctx.counters.modelUsage['glm-5.3-flash']).toMatchObject({ inputTokens: 150, outputTokens: 300, costUsd: expect.closeTo(1.5, 5) });
  });

  it('第一轮折进去的按件工具费（generate_image $0.20）不会被第二轮覆盖掉', () => {
    const ctx = mkCtx();
    ctx.counters.modelUsage = { 'glm-5.3-flash': { inputTokens: 10, outputTokens: 10, cacheReadTokens: 0, cacheCreateTokens: 0, costUsd: 0 } };
    ...
    ctx.counters.modelUsage = { 'glm-5.3-flash': { inputTokens: 5, outputTokens: 5, cacheReadTokens: 0, cacheCreateTokens: 0, costUsd: 0.01 } };
  ...
  it('carry 是快照不是引用：存完再改 counters 不影响它', () => {
    ...
    fill(ctx, 100);
    const carry = ctx.takeCarry();
    ctx.counters.modelUsage['glm-5.3-flash'].inputTokens = 999;
    expect(carry.modelUsage['glm-5.3-flash'].inputTokens).toBe(100);
  });
