...
const MODE_ALWAYS = [
  ...
  '告一段落的线折起来',   // 08-27 收纳器：roll 教义（擦是毁掉，折起是收着）
  '正文在板上，侧栏一句',   // 08-27：防双倍输出
  ...
  '位置和线是同一种语言',   // 08-27 用户拍板：摆放和连线相辅相成，不是两件事
  '一件事的板上动作在同一回合连着做',       // 08-30 board_batch 暂停后的接棒教义
  '## 硬规则',
  '派干活型子代理时显式写',
  '画布也是黑板',
  'report_issue',
  '找回他当时的原话时，用 `read_user_messages`',   // 09-17 数据丢失 1b：回退 / 中断后读用户原话，两种模式都要教
  '## 跟用户说话',
];
