...
const RP_ONLY = [
  '这个项目是演出模式',
  '演故事时让别人替你写',
  '你永不入戏',             // 08-27 用户拍板：主 agent 只有旁白笔，台词一律归角色
  '一句话的过场，旁白带过',   // 08-27 龙套口子：背景板不铸角色
  '从酒馆卡 / 预设里搬角色',
  '笔权不用写',   // 08-27：混笔病 —— 台规钉卡尾，GM 只管选笔和用 cue 纠偏
  '把下一步做成可按的',   // 08-27：编排线的控件教义（分支拍/节奏钮/点角色/骰）
  '开场先把玩法问出来',   // 08-27 用户拍板：开场必问（AskUserQuestion 上板选项），中途想换直接说
  '每种玩法有自己的版面',   // 08-27 四模式版式：rounds 桌机器排/free 星群/directed 分镜带/solo 板是他们俩的
  '台上有自动广播，你不用转发',   // 08-28 转发机：话落板在场角色自动收到，GM 别再替机器转述
  '没设场 = free + 台上广播',     // 08-28：忘了 set_scene 角色也活着（默认广播兜底）
  '用于**召回散场的角色**',       // 08-28：SendMessage 主用途=召回（对挂着的角色迟到 300s；召回 5 秒内醒）
  '每拍旁白都把场况填进 `facts`',   // 08-28 文风节食：机器投干货不投散文（用户拍板走工具参数不走 helper）
  '第一句话也归它自己',             // 08-28：开场白归角色 —— GM 替说第一句，角色整场学 GM 的腔
  '落了板的才是公开台词',           // 08-28 对账：私语（keep=false）不落板不广播，GM 看不到≠故障
  '引用他的原话',                   // 08-28 中介路由：rounds/directed 用户公开发言经 GM 转交，禁改写
  '角色自己散场不是事故',           // 08-28 对账：「不该收回合」的合法例外 —— 空等两次自动散场
  '回声有阻尼',                     // 08-28 对账：MAX_HOP 静音写进教义，GM 不再误判「角色没反应」
  // 08-28 晚·演员位重构（会话中途注册 agent 在 resume 架构下不可靠 → 预注册演员位）
  '登卡 → 立刻上场',                // 起角色两步同回合做完，「等一回合」退役
  'subagent_type: "rp-actor"',      // 演员位派发形状（name = 收件地址）
  '自动续进自己的线',               // 预制摆位：open_lane 以角色名开专线，角色只管补台词
  '选项就是下一幕的门',             // 幕间私聊不推进：用户没按控件前 GM 不动
  '状态是场务的正事',               // 状态板 set_text 每拍更新 + 自定义配件 + board_batch 一车
  '尽量原地照搬，别转写',           // solo 对象卡整段 cp，不洗掉用户挑中的声音
  '角色出选项 = 你发的许可',        // 控件语法 GM 按需发放（用户拍板：不是每个角色都揣按钮上台）
  '处理物落盘，别用完就丢',         // 预设消化产物进 预设/<名>/（落点对账+未启用条目，用户随时取用）
  '板的眼睛是 `look_at_board`',     // 08-28 补缝：rp 工具清单切换段（545-550）此前无特征串覆盖
  // 08-28 晚·Izumi 预设消化（proj_mtcyfdls 哥杀会话诊断后的沉浸感五刀）
  '世界不迎合玩家',                 // 难度三档（爽/写实/严酷），写实起 NPC 不给面子
  '重要抉择和转折点必停笔等他',     // 代笔档（全代入/大纲），扩写只在大纲档且抉择必停
  '导演话',                         // IC/OOC 分层：指令提问不进戏、不改写成台词
  '每拍旁白收尾都给一个决定点',     // 每拍把手 + 选项三味配方 + 禁正文括号舞台指令
  '谈话戏连着两拍没有判定',         // 节奏上钟 + 无聊过场跳时间 + 旁白拍尺寸
];
const MODE_ALWAYS = [
  '版面按线组织',   // 08-27 空间规划：lanes 教义在共享的黑板节，两种模式都要有
  '演完/翻篇的线收进卷',   // 08-27 收纳器：roll 收卷教义（擦是毁，收是藏）
  ...
  '落位直觉',                 // 08-27：side 自动挑侧 + 接楼方向跟用户的手
  '位置和线是同一门语言',   // 08-27 用户拍板：摆放和连线相辅相成，不是两件事
  '板面维护按拍打包',       // 08-27 batch 重置的配套教义
  '## 硬规则',
  '派干活型子代理时显式写',
  '画布也是黑板',
  'report_issue',
