...
const RP_ONLY = [
  '这个项目是演出模式',
  // 08-30 用户拍板：角色子代理暂时停用，台上所有人由主 agent 代演。旧断言里凡是钉
  // 「派角色 / 点名 / 等它写完」那套编排的一律删掉（分工、交给角色那四段、rp-role
  // 派发、open_lane 一人一线、重派顶掉、情绪不要给）—— 它们描述的动作现在明令禁止，
  // 留着断言等于把停用令锁死不了、还把旧教义焊在文件里。
  '你一个人演全场',                     // 新核心：环境和所有人的台词都是你的笔
  '这一版不要派角色子代理',             // ⛔ 停用令本身（写成断言，改稿时漏不掉）
  '所有人一个腔',                       // 代演的真风险，防它靠开口前读档案
  '写完卡这件事就完了',                 // cast_role 语义降级：只写档案，不接着派
  '一句话的过场，你自己带过',           // 龙套口子：背景板不写卡
  '开场先问清楚三件事',                 // 想跟谁演 / 难度 / 代笔档
  '世界不迎合玩家',                     // 难度三档，写实起路人不给面子
  '重要抉择和转折点必须停笔等他',       // 代笔档：扩写只在大纲档且抉择必停
  '戏外的话',                           // 戏里/戏外分层：指令提问不改写成台词
  '一个字别改写',                       // 用户写的戏文原样落板
  '只复述不评价',                       // 扩写的分寸：铺开是服务，改意思是抢戏
  // ── 缺省文风（08-30 从用户给的酒馆预设里提炼，落进系统提示词当自带缺省）──
  '缺省文风',
  '前后不跟声音描写',                   // 对白只写内容不写声音（任何文风下都成立）
  '别把"白描/克制/Show don\'t tell"当成普世真理',  // ⭐ 08-30 更正：它是文风之一，不是地板
  'story-craft',                        // 剧情技法同上
  '有限视角',                           // 人物只知道被告知的和眼前的
  '别猜人设',                           // 设定没写的性格不现编
  '想到的头两个走向先毙掉',             // 第一反应都是公式
  '镜头可以切走',                       // 用户角色没事干时别围着他凑字数
  '动笔前先想一遍',                     // 拍前七问
  '不该知道什么',                       // 七问里最花心思那一条：信息差
  // ── 版面与场务（子代理退场后仍然全套有效）──
  '把下一步做成可按的',                 // 控件教义（分支/常设面板/点场/骰）
  '每一拍收尾都给一个决定点',           // 把手 + 选项三味配方 + 禁正文括号舞台指令
  '选项就是下一幕的门',                 // 用户没按之前戏停在这一拍
  '状态是场务的正事',                   // 状态板 set_text 每拍更新 + 同回合连着做
  '谈话戏连着两拍没有判定',             // 节奏上钟 + 无聊过场跳时间 + 一拍别超一屏
  '别自己编骰子点数',                   // roll_dice 真投，编的会塌掉整桌信任
  '从酒馆卡 / 预设里搬东西',
...
const MODE_ALWAYS = [
  ...
  '结尾不抒情、不总结',
  '这六条是**地板不是风格**',
  'story-voice',   // 地板那节点名了它（文风不是地板，去 skill 挑），两个模式都看得见
  ...
  '位置只说关系，永远不报像素',   // 08-29 刀 E：先切块再填，不是写一条想一条
  '别压在第三块身上',         // 08-29：走廊偏好退役成 agent 手摆纪律（压住谁返回会报）
  '一行 26 个汉字',           // 08-29 刀 D：护栏从像素改成字数（agent 手里的东西是字）
  '版面配方',                 // 08-29 刀 E：情景摆放指导（站主点名"个中情景的摆放指导"）
  '自己定几个空位，分段填进去', // 08-30 用户拍板：分段语义权归 agent，机器只包几何（flow 降为兜底）
  '装不下不丢字，落暂存架等你安置',   // 08-29 刀 E 立的上限纪律，08-31 处置改成溢出暂存
  '一张纸规划一次，满了自己开下一页',   // 08-29 刀 F：不替它翻页（站主"每张纸规划一次"）
  '有限制、有规划的自由',     // 08-29 刀 F 站主定调：纸上哪儿都能放，但落笔前要规划
  '先把这张纸用足再翻页',     // 08-29 刀 F：满 = 怎么调整都放不下，不是"最后一件下面没地方"
  '那是兜底不是版面',         // 08-29 刀 F：机器帮忙找地方 ≠ 机器定版面
  'open_sheet{ title:"第一章 · 便利店", plan:[',   // 站主点名要示例，别只讲规矩
  '什么时候翻纸',             // 08-29 刀 C：换纸两判据（放不下 / 换话题）
