 ...
 * 就说明排在它前面的字段都已闭合**（所以 write_on_board 的 schema 把 place/near/
 * reply_to 排到了 text 前面）。也就是说框在第一个字到达的那一刻立起来，正好。
 *
 * 2026-09-05 意图层：spot 里不再有像素，只有关系（place.by / near / reply_to）。
 * 前端拿到的是「贴着谁」，把直播框立在那件东西旁边；真位置由服务端求解器落。
 */
export const TOOL_INPUT_STREAM_FIELDS = {
  ...
  mcp__nodesign__write_on_board: { field: 'text', spot: ['place', 'near', 'reply_to', 'chain', 'tag', 'width'] },
  ...
  // 换动作时发 reset 让前端另起一张。
  mcp__nodesign__board_batch: { batch: 'write_on_board', field: 'text', spot: ['place', 'near', 'reply_to', 'chain', 'tag', 'width'] },
};
 ...
 *
 * ⚠️ 一个位置字段都没有时返回的是**空对象不是 null**（2026-08-29 刀 F）：
 * "什么都没点名" 本身就是一条位置信息 —— 那是**纸内顺排**，前端照服务端同一条
 * 规则算得出落点。返回 null 的话这条事件根本不发，前端只能把字先写在视口一块
 * 空地上、等落盘再跳过去（站主看到的"流式完毕之后再移动"）。而顺排恰恰是最
 * 常见的调用形态。
 *
 * @returns {object|null} null 只在入参根本不是对象时出现
 */
export function pickSpot(input, fields) {
  ...
  for (const k of fields) {
    ...
    if (typeof v === 'string' || Number.isFinite(v) || typeof v === 'boolean') out[k] = v;
  }
  return out;
}
