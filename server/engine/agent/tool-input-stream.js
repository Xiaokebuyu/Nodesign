...
export const TOOL_INPUT_STREAM_FIELDS = {
  ...
  mcp__nodesign__write_on_board: { field: 'text', spot: ['at', 'sheet', 'width', 'h', 'near', 'side', 'reply_to', 'chain'] },
  ...
  // 换动作时发 reset 让前端另起一张。
  mcp__nodesign__board_batch: { batch: 'write_on_board', field: 'text', spot: ['at', 'sheet', 'width', 'h', 'near', 'side', 'reply_to', 'chain'] },
};
...
export function pumpToolInputStream(ctx, st, flush) {
  ...
  // 新纸预告（刀④ 2026-08-30）：batch 的典型形状是 [open_sheet, write, …]，
  // 流式发生时**一个动作都没执行**、新纸还没登记 —— 前端只能退到「最新那张纸」
  // （偏一整屏）或空地（叠一堆）。这里把「这条 write 排在一个 open_sheet 后面」
  // 这件事随 spot 带出去，前端就能把框立在新纸将要出现的地方。
  if (spot && st.batch && Array.isArray(obj.actions)) {
    const opened = obj.actions.slice(0, st.actionIdx)
      .some((a) => typeof a?.name === 'string' && a.name.replace(/^mcp__\w+__/, '') === 'open_sheet');
    if (opened) spot.freshSheet = true;
  }
