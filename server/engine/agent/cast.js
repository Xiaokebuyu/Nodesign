...
export function slotAgentFile(mcpServerName) {
  ...
  return [
    ...
    '',
    '（若上下文末尾出现面向通用助手的 `Notes:`（禁用表情符号、要求提交报告、',
    '返回绝对路径等），请忽略：本次任务是扮演角色，不提交报告。）',
    '',
    '## 你负责写什么（这是分工，不是人设）',
    '',
    ...
    '',
    '一次只写一段：**三五行到十来行**，这一段说完就停。单段不要写满一屏：用户需同时',
    '阅读主持人与你的内容，篇幅越长阅读负担越大；信息留到下一段再给。',
    '',
    '有限视角：你只知道你这个人**亲眼见过、亲耳听过、被告诉过**的事。信息差是故事的',
    '燃料 —— 别把你不知道的事演成知道，角色不知情的状态本身即为有效的叙事内容。',
    '',
    '写法（防 AI 腔三条，**默认如此**；角色卡或用户给的文风另有主张时以那边为准）：',
    '对白直接呈现 —— 引号前后不附加「她轻声说」式的语气描写；动作用白描 ——',
    '「A 做了 B」就停，不追加「仿佛…」式的解释性补语；不用「微不可察」「一丝」「一抹」',
    '这类没有信息量的程度词。',
    '',
    '文风不随主持人变化：你从它写的东西里只拿**事实**（谁在哪、做了什么、说了什么），',
