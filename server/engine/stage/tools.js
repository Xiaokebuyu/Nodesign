...
const choiceSchema = z.object({
  ...
  hint: z.string().max(60).optional().describe('按钮下面那行小字：说清楚点下去他具体会做什么、说什么，或者接下来会发生什么。⛔ 不许写类别词 —— "主线""人际""意外""推进剧情"这类词对玩家无意义，写了显示器会直接删掉'),
...
export function createStageTools(ctx) {
  ...
  const writeScene = tool(
    ...
    + '每一枚都写玩家具体会做什么。**选项接在你这一段收笔时的处境上**（谁在场、刚发生了什么、时间走到哪），不是接玩家刚说的那句话 —— 正文写完回头看一眼结尾再写选项；正文里已经过去的关口（她已经提了词、铃已经响了）不得再作为选项给出。**没有选项这一段就没写完**。'
    + 'state 每段必填：这一段改了哪些状态值就报哪些，什么都没变就传空数组 —— 空数组表示"已核对，无变化"。',
    {
      text: z.string().min(1).max(8000).describe('这一段的正文'),
      choices: z.array(choiceSchema).min(1).max(5).describe('留给玩家的可选行动，两到四项'),
      ...
      state: z.array(z.object({
        ...
        .describe('这一段改了的状态值，只传变了的键：[{"key":"好感","value":32}]。没变就传 []（必填，用于确保每段都核对数值）'),
    },
  ...
  const forget = tool(
    'forget',
    '删掉一条记错了或者已经作废的记忆。剧情推翻了旧设定时用，不要同时保留两份相互冲突的记录。记在某个人卡上的要带 who。',
    {
  ...
  const rollDice = tool(
    'roll',
    '判定骰。服务端真随机，⛔ 不得自行编造点数：编造会使判定机制失去可信度。给了 dc 机器直接报成败（单颗 d20 及以上天然 20 / 1 是大成功 / 大失败）；'
    + '什么时候掷：结果不确定且有代价的行动，玩家明确宣告了冒险的动作。日常琐事不掷。失败应换来信息或代价，成功应带出新问题；不做无后果的判定。',
    {
  ...
  const illustrate = tool(
    'illustrate',
    '给故事配一张图。每张计入玩家额度并耗时约一分钟，**只在关键转折、情感最重的一刻、值得记住的画面**上用，平常段落一张不画；两张之间至少隔三段。'
