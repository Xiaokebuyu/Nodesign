...
export async function composeStagePrompt(wsRoot, root, stored) {
  ...
  if (panels.length) {
    ...
    parts.push('## 面板\n这个故事有这几块清单，数量账全在这里记、不进正文：\n'
      ...
      + '\n得到、用掉、穿上、标价、卖出，**先 update_panel 再 write_scene**（write_scene 一返回这一轮就结束）。玩家自己在显示器里买了、用了东西，会以【便条】告诉你，在正文中承接即可。每句话末尾「面板：…」是机器报的现况。');
  }
  if (stored?.images?.allow) {
    parts.push('## 配图\n'
      + '玩家允许你给故事配图（illustrate）。每张会计入玩家的每日额度并耗时约一分钟，因此**需克制使用**：只在关键转折、情感最重的一刻、或一个值得记住的画面上画，'
  ...
  parts.push(
    ...
    + '不可逆的变化用 remember 记一条：某个人记得的事带 who 写进他的卡，这个故事的事不带。'
    + '\n\n**判定**：结果不确定且有代价的行动才掷（翻墙、说服、潜行、出手、赌运气），日常琐事不掷。两条路：'
    + '玩家已经宣告了动作 → 你 roll（给 dc 机器报成败）再写；你给的选项本身是一次冒险 → 在 choices 里带 check（名目 + 难度），玩家点下去机器代掷，成败随他那句话以【判定】告诉你。'
    + '成败一律照机器报的写，不改判、不重掷。失败应换来信息或代价，成功应带出新问题；不做无后果的判定。设定里写了骰子规则（d20 还是 d100、有没有修正）就按设定的。'
