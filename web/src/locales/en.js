...
export default {
  ...
  '内部 id': 'Internal id',
  '自动': 'Auto',
  // 同名顶替提示（09-09）：与内置 API 行同名 = 有意顶替；与内置 Claude 订阅名同名 = 服务端会拒
  '已顶替内置的「{id}」：这个模型的请求现在走您自己的 API Key，不经过站点。': 'Overrides the built-in "{id}": requests for this model now use your own API key and do not go through the site.',
  ...
  '删除项目': 'Delete project',
  // 回收站（09-17）：删除确认框写保留期，toast 带撤销，首页「最近删除」
  '删除「{name}」？项目会移到「最近删除」，{days} 天内可以恢复，之后自动永久删除。': 'Delete "{name}"? It moves to Recently deleted and can be restored within {days} days, after which it is permanently deleted.',
  ...
  '永久删除「{name}」？工作区里的文件会立即清除，无法恢复。': 'Permanently delete "{name}"? Its files are removed immediately and cannot be recovered.',
  '已永久删除「{name}」': 'Permanently deleted "{name}"',
  '永久删除失败：{err}': 'Permanent delete failed: {err}',
  '删除的项目在这里保留 {days} 天，到期自动永久删除。': 'Deleted projects stay here for {days} days, then are permanently deleted.',
  '最近没有删除的项目。': 'No recently deleted projects.',
  '删除于 {when}': 'Deleted {when}',
  '即将永久删除': 'Deleting soon',
  '{n} 天后永久删除': { one: 'deleted permanently in {n} day', other: 'deleted permanently in {n} days' },
  ...
  '做一张海报：「城南灯会 · 正月十五 · 护城河畔」，暖红色，竖版': 'Make a poster: "Southside Lantern Fair · 15th night · by the moat", warm red, portrait',
  '把这三件整理成作品介绍页：一个天气 App、一套咖啡店 VI、一支 30 秒动画短片': 'Turn these three into a portfolio page: a weather app, a coffee-shop brand kit, a 30-second animated short',
  ...
  '新项目默认放在 NoDesign 自己的目录里。想放进你的文件夹，或在一个已有仓库里干活：': 'New projects live in NoDesign\'s own data folder by default. To keep one in your own folder, or work inside an existing repo:',
  '第一条消息发出去，就在这个文件夹里开工：': 'Your first message starts work in this folder:',
  '换一个': 'Change',
  '改回放在 NoDesign 自己的目录里': 'Back to NoDesign\'s own data folder',
  '只是先定位置：不会建项目，也不会替你发话': 'Only sets the location: no project is created and nothing is sent on your behalf',
  '选一个文件夹': 'Choose a folder',
  '创建中…': 'Creating…',
  '发送（Enter）': 'Send (Enter)',
  '开 工 中': 'Starting…',
  '开 工': 'Start',
