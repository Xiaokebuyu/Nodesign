/**
 * 物件动作表兑换（2026-08-27 从 BoardObject 抽出）。
 *
 * 按钮清单由形态表给（board-kinds.js 的 actions，顺序即渲染顺序），这里只把
 * 动作 id 兑换成图标和回调。眼下唯一的消费方是 BoardObject 的 hover 工具条
 * （点选操作条同日建又同日撤：单击语义改成直接开标注），但兑换表留在这里 ——
 * 同一件东西两个实例是这个仓库最贵的一课，将来再长第二条工具条也从这份取。
 *
