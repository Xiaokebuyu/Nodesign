...
const PROVIDER_PRESETS = [
  ...
  { id: 'zhipu', label: '智谱（Z.ai）', baseUrl: 'https://open.bigmodel.cn/api/paas/v4', protocol: 'openai-chat', brand: 'glm', example: 'glm-5.3 / glm-5.3-flash' },
  ...
  { id: 'zen', label: 'OpenCode Zen', baseUrl: 'https://opencode.ai/zen/v1', protocol: 'openai-chat', brand: 'opencode', example: 'big-pickle（免费 stealth 行只在这个入口）' },
  { id: 'zen-go', label: 'OpenCode Zen Go（Go 订阅）', baseUrl: 'https://opencode.ai/zen/go/v1', protocol: 'openai-chat', brand: 'opencode', example: 'glm-5.3-flash / deepseek-v4-flash-vision-exp' },
...
function NumberPick({ value, presets, allowEmpty, emptyLabel, onChange, width = 120 }) {
  ...
  const options = [
    ...(allowEmpty ? [{ value: '', label: emptyLabel || '默认' }] : []),
  ...
  return (
    <div style={{ display: 'flex', gap: GAP.xs, alignItems: 'center' }}>
      ...
      {custom && <TextInput type="number" width={110} value={value ?? ''} onChange={(v) => onChange(num(v))} placeholder="数字" />}
    </div>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: GAP.lg }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: GAP.md, marginBottom: GAP.sm }}>
          <span style={{ fontFamily: FONT_SANS, fontSize: FONT_SIZE.sm, color: COLOR.text2 }}>① 服务商（接口地址 + 钥匙）</span>
          <Btn small onClick={addUpstream}><Plus size={12} /> 加一个</Btn>
        </div>
        {Object.keys(upstreams).length === 0 && <Hint>先加一个服务商：从预设里挑（DeepSeek / OpenAI / 中转站…），填上钥匙；然后在 ② 里加模型。</Hint>}
            ...
            return (
              <Card key={id} style={{ padding: GAP.md }}>
                <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 200px 28px', gap: GAP.sm, alignItems: 'end' }}>
                  <Field label="服务商">
                    <Select value={preset?.id || (u.baseUrl ? 'custom' : '')} options={[{ value: '', label: '选一个…' }, ...PROVIDER_PRESETS.map((p) => ({ value: p.id, label: p.label }))]} onChange={(v) => applyPreset(id, v)} />
                  </Field>
                  <Field label={`接口地址（${(u.protocol || 'openai-chat') === 'openai-chat' ? 'OpenAI 格式，带 /v1' : 'Anthropic 格式，不带 /v1'}）`}>
                  ...
                  <Field label="API Key">
                    <TextInput type="password" value={u.key || ''} onChange={(v) => setUp(id, { key: v })} placeholder={u.keyEnv ? `从 env ${u.keyEnv} 取` : (u.authStyle === 'none' ? '本机服务不用钥匙' : '钥匙')} />
                  </Field>
                ...
                <Fold title="高级" desc="协议 / 鉴权方式 / 内部名">
                  <div style={{ display: 'grid', gridTemplateColumns: '160px 160px 160px', gap: GAP.sm, alignItems: 'end' }}>
                    ...
                    <Field label="鉴权头">
                      <Select value={u.authStyle || ''} options={[{ value: '', label: '按格式默认' }, ...enums.AUTH_STYLES]} onChange={(v) => setUp(id, { authStyle: v || undefined })} />
                    </Field>
                    <Field label="内部名（模型行引用它）">
                      <TextInput value={id} onChange={(v) => renameUp(id, v)} placeholder="字母数字" />
                    </Field>
                  </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: GAP.md, marginBottom: GAP.sm }}>
          <span style={{ fontFamily: FONT_SANS, fontSize: FONT_SIZE.sm, color: COLOR.text2 }}>② 模型（每一行 = 模型选择器里的一项）</span>
          <Btn small disabled={!Object.keys(upstreams).length} onClick={addModel}><Plus size={12} /> 加一行</Btn>
        </div>
        <div style={{ display: 'grid', gap: GAP.md }}>
          {models.map((m, i) => {
            const pr = probe[m.id];
            ...
            return (
              <Card key={i} style={{ padding: GAP.md }}>
                <div style={{ display: 'grid', gridTemplateColumns: '170px 1fr 1fr 150px 28px', gap: GAP.sm, alignItems: 'end' }}>
                  <Field label="服务商">
                    <Select value={m.upstream || ''} options={[{ value: '', label: '选一个…' }, ...Object.keys(upstreams).map((k) => ({ value: k, label: upstreams[k].label || k }))]}
                  ...
                  <Field label="模型名（发给服务商的 model，一字不差）">
                    <TextInput value={m.wireModel} onChange={(v) => setModel(i, { wireModel: v, ...(idAuto ? { id: idFromWire(v) } : {}), ...(!m.label || m.label === m.wireModel ? { label: v } : {}) })} placeholder={preset?.example || '如 deepseek-chat'} />
                  </Field>
                  <Field label="显示名（选择器里的名字）">
                    <TextInput mono={false} value={m.label} onChange={(v) => setModel(i, { label: v })} placeholder="如 DeepSeek V3" />
                  </Field>
                  <Field label="上下文窗口">
                ...
                <Fold title="高级" desc="说明 / 思考参数 / 输出上限 / 图标 / 内部 id">
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px 140px 200px 140px 160px', gap: GAP.sm, alignItems: 'end' }}>
                    ...
                    <Field label="reasoning_effort"><Select value={m.reasoningEffort || ''} options={[{ value: '', label: '不传' }, ...enums.REASONING_EFFORTS]} onChange={(v) => setModel(i, { reasoningEffort: v || undefined })} /></Field>
                    <Field label="单轮最大输出"><NumberPick value={m.maxOutput} presets={MAX_OUTPUT_PRESETS.map((v) => ({ value: v, label: v >= 1024 ? `${Math.round(v / 1024)}k` : String(v) }))} allowEmpty emptyLabel="默认" onChange={(v) => setModel(i, { maxOutput: v })} width={110} /></Field>
                    <Field label="图标"><Select value={m.brand || 'custom'} options={enums.BRANDS} onChange={(v) => setModel(i, { brand: v })} /></Field>
