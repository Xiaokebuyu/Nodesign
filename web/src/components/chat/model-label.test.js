 ...
 * （`GLM-5.3-Flash · 官方直连（限时免费）` 和 `GLM-5.3-Flash · Merge 网关`），
...
const OPTS = [
  { id: 'a', label: 'Sonnet 5' },
  { id: 'b', label: 'Opus 5' },
  { id: 'c', label: 'GLM-5.3-Flash · 官方直连（限时免费）' },
...
describe('模型短名', () => {
  ...
  it('⭐ 第一段撞名时留两段 —— 两条不同的线不许显示成同一句话', () => {
    expect(c('GLM-5.3-Flash · 官方直连（限时免费）')).toBe('GLM-5.3-Flash · 官方直连');
    expect(c('GLM-5.3-Flash · Merge 网关')).toBe('GLM-5.3-Flash · Merge 网关');
    expect(c('GLM-5.3-Flash · 官方直连（限时免费）')).not.toBe(c('GLM-5.3-Flash · Merge 网关'));
  });

  it('撞名的那两条**也要**去掉括号（短名的意义就在这儿）', () => {
    expect(c('GLM-5.3-Flash · 官方直连（限时免费）')).not.toContain('（');
  });
