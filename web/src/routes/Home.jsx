 ...
 */
/**
 * 首页示例 —— **四条全部自带内容**（09-08 重写）。
 *
 * ## 为什么重写
 *
 * 依据是 `~/claude-report-file/0908-churn-analysis.md`：老那条
 * 「给我喜欢的歌做一个歌词视觉页」**没说是哪首歌**，agent 只能反问，
 * 而用它的 9 个人里 **7 个在反问处离开**。四条老示例都是这个毛病：
 * 需求只有五到十个字，agent 拿不到可对齐的材料，于是第一轮变成一串问题。
 *
 * 对照数据：答了问卷的 7 个人**全部**继续做下去，4 个当场出产物。
 * **问题不在问卷，在「第一次来的人手上没有内容可给」。**
 *
 * ## 改的是示例，不是教义
 *
 * ⛔ 站主 09-08 明确否决过「首轮不问先做」—— 「先对齐再动手」不动。
 * 这四条走的是另一条路：**把内容直接放进示例里**，agent 就没有什么要问的了。
 * 顺带它还示范了正确的用法：把你自己的词/设定/清单贴进来，而不是描述一个愿望。
 *
 * ## 四条的覆盖
 *
 * 按跑过的 68 人首句题材分布来配：站点/页面 21、图片/海报 10、RP/角色 8、作品集。
 * ⛔ 不许再出现「演示 deck」—— deck 09-08 已并入站点，不再是独立形态。
 */
const EMPTY_EXAMPLES = [
  '把这几句做成一页歌词视觉：「晚风吹过便利店的招牌／我数着找零走回家／今天也没什么大事发生」',
  '做一个角色档案站，先放这一个：秦晚，26 岁，旧钟表店老板，话少，怕猫',
  '做一张海报：「城南灯会 · 正月十五 · 护城河畔」，暖红色，竖版',
  '把这三件整理成作品介绍页：一个天气 App、一套咖啡店 VI、一支 30 秒动画短片',
];
...
function EmptyState({ onPick }) {
  return (
    <div className="ndd-sheet">
      ...
      <div className="chips">
        {EMPTY_EXAMPLES.map((zh) => {
          // 点进输入框的是**用户看到的那句**（英文界面下就是英文那句），不是中文原文
          const text = t(zh);
          return <button key={zh} onClick={() => onPick?.(text)}>{text}</button>;
        })}
      </div>
    </div>
  );
