...
/* ⭐ 950 压在顶栏之上，为的是「屋里的光暗了，画框没理由还亮着」——
   只照桌子不照顶栏，夜里一眼看出是贴上去的一层。留在 FloatingPanel（9998）之下。

   ⛔⛔ **但「压在它上面」不等于「往它脸上画」。** 这一层认识的东西只有桌上的纸，
   顶栏、横幅、以前那些弹窗在它眼里全是桌面：影子照投、台灯的光池照铺，
   于是一块不透明的横条上盖着一层带梯度的光，读起来就是半透明（站主 09-06 报
   模型选择器、09-07 报顶栏，同一件事）。
   ⭐ 09-07 起改由**遮挡图**来说「屏幕上哪一块不是桌子」（蓝色通道），说是画框的
   地方这一层整段让开、只留一层均匀的夜色。z-index 一个字都没动 ——
   靠抬 z 躲这一层是治不完的（Modal 800→960、Popover→9600 之后还有顶栏和横幅）。
   见 home-occluders.js 的 CHROME 那段。 */
