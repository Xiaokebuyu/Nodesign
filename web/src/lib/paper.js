...
const BASE = {
  ...
  red:    '#A8362B',
  /** 用户的笔（2026-09-17 板书树）：钢笔蓝黑。墨是 agent 写的，蓝是用户写的 ——
   *  画布上第一次有「谁在说话」的视觉区分。纸底上对比度 8.07/9.16/7.94，稳过红（5.17）、浅于墨（13.73），
   *  读起来像另一只笔写在同一张纸上，不像界面色。⚠️ 金（#C9A227）对比度只有 2.11，不能写正文，只做记号。 */
  blue:   '#2C4A63',
  /** 弹窗背后那层压暗：暖的，不是中性黑 */
...
/**
 * 工具面 —— 浮在画布上的**工具**表面（2026-08-07 起是墨面；2026-09-12 改成纸）。
 *
 * 08-07 的理由是「内容是纸，工具就不能也是纸，否则读成又一张卡」，做法是墨的反相：
 * 深色半透明 + 14px 圆角 + 两层模糊投影。印刷风改版之后这条不成立了 ——
 * 全站只剩它还是圆角深色玻璃，站主看样张选了 A「牛皮纸条」：工具条也是纸，
 * **靠实墨描边、直角和当前工具的反白块跟内容分开**，不靠换一种材质。
 *
 * ⚠️ 名字 09-12 从 INK_SURFACE 改成 TOOL_SURFACE：它说的是「工具的面」，不是「墨面」。
 *
...
export const TOOL_SURFACE = {
  /** 面：就是纸。工具条 = 摆在画布上的一条牛皮纸 */
  bg: PAPER.paper,
  /** 组与组之间那道浅线（同产物卡头那条 rule） */
  hair: '#C7B79A',
  /** 面上的字与图标（纸上的正文墨，= COLOR.text2） */
  text: '#4A3E31',
  /** 次要态（不可用 / 说明字，= COLOR.sub） */
  textDim: '#A39882',
  /** hover 底：一层很淡的墨 */
  hover: 'rgba(43,33,23,0.07)',
  /** 当前工具：实墨块反白（墨面时代是暖棕压在深色上，纸上读不出来） */
  active: INK_EDGE,
  /** 当前工具上的字 */
  activeText: PAPER.paper,
  /** 一整条纸的硬边 + 不模糊的错位影，跟画布上的卡同一套 */
  shadow: PAPER_SHADOW.mid,
};
