 ...
 * 一个秋天的下午。
 *
 * 台灯不动：它就钉在桌上那个位置，比太阳低一点 —— 灯照的是你正在读的那张纸，
 * 不是天花板。
 *
 * ⭐⭐ 09-05 从 (0.72, 0.18) 挪到 (0.82, 0.16)：**落脚点要落在纸外面的桌面上**。
 *   原来那个点在输入纸的右上角**里面**。点光源的影子是从落脚点向四周放射的，于是
 *   纸把影子往右甩到自己右边沿外面、往上甩到签上 —— 离灯最近的东西反而最黑
 *   （站主 09-05：「既然灯在右上方，为什么输入框右上方还有阴影」）。而且灯最亮的
 *   那一块被纸整张盖住，桌面上只露出光池的边，读起来"有点暗"。挪到纸外面，亮心落在
 *   看得见的桌面上，纸整张都在落脚点的左下方，影子只往左下走。
 *   ⚠️ 手机上纸几乎占满整宽，落脚点仍会落回纸里 —— 那种情形靠着色器里 far∝距离
 *   兜住（纸底下正对着灯的地方不往外投影），见 home-canopy-glsl.js castShadow。
 */
export const SUN_FROM = [0.78, 0.14];
export const LAMP_AT = [0.82, 0.16];
