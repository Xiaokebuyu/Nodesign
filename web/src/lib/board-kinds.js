...
export const ARTIFACT_PREVIEW_H = { deck: 360, site: 400, docx: 420, browse: 360, stage: 400, repo: 300 };  // docx 是竖版 A4，给高一点；stage 取一屏戏；repo 是一列文件树
...
export const KINDS = {
  ...
  /**
   * 仓库卡（2026-09-08 存量仓库道）。本地版打开用户自己的文件夹当项目时，桌面缩进
   * `<folder>/.nodesign/`，他的东西不在桌面上 —— 这张卡是看向文件夹的一扇窗。
   *
   * 是 runtime backing 而**不是文件夹卡**：文件夹卡的守则是「画布层级即磁盘层级、拖进去
   * 等于真搬文件」，而用户的仓库一个字都不能被画布搬动。视图形态没有这条守则。
   * 窗里：文件树 + git 状态 + 代码阅读器；卡上：分支、改动数、一列顶层条目。
   */
  repo: {
    label: '仓库',
    category: 'tool',
    ...
    chrome: 'card',
    card: 'artifact',
    size: artifactCard(ARTIFACT_PREVIEW_H.repo),
    reader: null,
    primary: 'open',
    actions: [],
    legacyBucket: 'art',
